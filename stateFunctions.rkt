...
(define (M_state_keyword_helper exp state return next break continue)
  (cond
    ((eq? 'var (keyword exp))
       (M_state_declare exp state return next))
    ((eq? '= (keyword exp)) (M_state_assign (cadr exp) (caddr exp) state return next))
    ((eq? 'if (keyword exp)) (M_state_if exp state return next break continue))
    ((eq? 'while (keyword exp)) (M_state_while (cadr exp) (caddr exp) state return next))
    ...
    ((eq? 'begin (keyword exp)) (M_state_block exp state return next break continue))
...
(define (M_state exp state return next break continue)
  (cond
    ...
    (else              (M_state_keyword_helper exp state return next break continue))))



; block statements
(define (M_state_block ls state return next break continue)
  ...
  (lambda (stmts state return next break continue)
    (if (null? stmts)
        ...
        (M_state (car stmts) state return
                 (lambda (nstate) (M_statementlist (cdr stmts) nstate return next break continue)) break continue)
        )))
...
(define (M_state_if_2 condition statement1 statement2 state return next break continue)
  (if (eq? (M_bool condition state return) 'error)
      'error
      (if (eq? (M_bool condition state return) 'true)
          (M_state statement1 state return next break continue)
          (M_state statement2 state return next break continue))))

; handles if when we only have 1 statement
(define (M_state_if_1 condition statement1 state return next break continue)
  (if (eq? (M_bool condition state return) 'error)
      'error
      (if (eq? (M_bool condition state return) 'true)
          (M_state statement1 state return next break continue)
...
; if operation
(define (M_state_if exp state return next break continue)
  (cond
    ((eq? (length exp) 3)
     (let* ((condition (cadr exp))
            (statement1 (caddr exp)))
       (M_state_if_1 condition statement1 state return next break continue)))
    ((eq? (length exp) 4)
     (let* ((condition (cadr exp))
            (statement1 (caddr exp))
       ...
       (if (eq? statement2 #f)  
           (M_state_if_1 condition statement1 state return next break continue)
           (M_state_if_2 condition statement1 statement2 state return next break continue))))
    (else 'error)))



; while operation
(define (M_state_while condition statement state return next)
      (loop condition statement state return next))

(define (loop condition statement state return next)
  (if (eq? (M_bool condition state return) 'true)
      ; Loop
      (M_state statement state return
               (lambda (s1) (loop condition statement s1 return next))
               (lambda (s1) (next s1))
               (lambda (s1) (loop condition statement s1 return next))
               ) 
      (next state)
