...
;(check-equal? (interpret "tests/p2_t2.bad") 164) ; fail
;(check-equal? (interpret "tests/p2_t3.bad") 32)
;(check-equal? (interpret "tests/p2_t4.bad") 2) ; fail
;(check-equal? (interpret "tests/p2_t5.bad") 'error) ; fail
...
; try catch finally state next break throw
(check-equal? (M_state_try 'try 'catch 'finally state 'next 'break 'throw) '())
