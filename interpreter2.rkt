...
(define (eval-operator expr enviroment throw)
  (eval-operator-cps expr enviroment throw (lambda (v) v)))

(define (eval-operator-cps expr environment throw return)
  (cond
    ((eq? '! (operator expr))
     (eval-expression-cps (operand1 expr) environment throw (lambda (r-op1) (return (not r-op1)))))
    ((and (eq? '- (operator expr)) (= 2 (length expr)))
     (eval-expression-cps (operand1 expr) environment throw (lambda (r-op1) (return (- r-op1)))))
    (else (eval-expression-cps (operand1 expr) environment throw (lambda (r-op1) (return (eval-binary-op2 expr r-op1 environment throw)))))))

; Complete the evaluation of the binary operator by evaluating the second operand and performing the operation.
(define (eval-binary-op2 expr op1value enviroment throw)
...
(define (catch-var catch-statement)
...
(define (lookup-in-frame var frame)
  (if (not (exists-in-list? var (variables frame)))
