...
(define update
  (lambda (var val environment)
    (if (exists? var environment)
        (call/cc (lambda (end) (
        (update-existing var val environment end)
        )))
...
(define update-existing
  (lambda (var val environment end)
    (if (exists-in-list? var (variables (car environment)))
        (cons (update-in-frame var val (topframe environment) end) (remainingframes environment))
        (cons (topframe environment) (update-existing var val (remainingframes environment) end)))))

; Changes the binding of a variable in the frame to a new value.
(define update-in-frame
  (lambda (var val frame end)
    (list (variables frame) (update-in-frame-store var val (variables frame) (store frame) end))))

; Changes a variable binding by placing the new value in the appropriate place in the store
(define update-in-frame-store
  (lambda (var val varlist vallist end)
    (cond
      ((eq? var (car varlist))
       (display (car vallist))
       (cons (begin (set-box! (car vallist) (scheme->language val)) end) (cdr vallist)))
      (else (cons (car vallist) (update-in-frame-store var val (cdr varlist) (cdr vallist) end))))))
